...
function BindGrid() {
    ...
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectSmmeAssessmentForEnterprise',
    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            var tableElement = $("#datatable-example");
            var select2Elements = $(".select2");

            var profileUrl = "/SMME/SMMEProfile?Id=";

            // Initialize select2 dropdown
            if (select2Elements.length) {
                select2Elements.wrap('<div class="position-relative"></div>').select2({
            ...
            if (tableElement.length) {
                var dataTable = tableElement.DataTable({
                    data: data,
                    columns: [
                        { data: null, defaultContent: "" }, // Responsive control
                        { data: "SMME_CompanyName" },
                        { data: "SMME_PrimaryContactNo" },
                        { data: "assign", defaultContent: "" }
                    ],
                    columnDefs: [
                        {
                            targets: 0,
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                var companyName = row.SMME_CompanyName || "N/A";
                                var contactEmail = row.SMME_PrimaryContactEmail || "No email";
                                var logo = row.SMME_Logo;
                                var smmeId = row.SMME_Id;
                                var initials = "";
                                var matchLetters = (companyName.match(/\b\w/g) || []);
                                if (matchLetters.length > 0) {
                                    initials = (matchLetters[0] + (matchLetters[matchLetters.length - 1] || "")).toUpperCase();
                                }

                                var avatarHtml = logo
                                    ? '<img src="' + logo + '" alt="Avatar" class="rounded-circle" width="40">'
                                ...
                                return '<div class="d-flex align-items-center">' +
                                    ...
                                    '<a href="' + profileUrl + smmeId + '" class="text-body fw-medium">' + companyName + '</a>' +
                                    '<small class="' + row.RegLbl + '">' + contactEmail + '</small>' +
                                    '</div>' +
                                    '</div>';
                            }
                        },
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                return '<div class="d-flex align-items-center">' +
                                    '<button class="btn btn-sm btn-icon me-2" data-bs-toggle="modal" data-bs-target="#shareProject" onclick="ShowAllAssessment(' + row.SMME_Id + ')">' +
                                    '<i class="ti ti-book ti-sm text-info"></i>' +
                                    '</button>' +
                                    '</div>';
                            }
                        }
                    ...
                    responsive: {
                        details: {
                            display: $.fn.dataTable.Responsive.display.modal({
                                header: function (row) {
                                    var name = row.data().SMME_CompanyName || "Details";
                    ...
                    order: [[1, "desc"]],
                    dom:
                        '<"row me-2"<"col-md-2"l><"col-md-10 d-flex justify-content-end"f>>' +
                        't' +
