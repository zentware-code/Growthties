...
!(function () {
    ...
    fns.length &&
     ...
     if (Id > 0) {
         ...
         //$('#rdoCurrentSession').trigger('change');
...
$("#ddlFinancialYearExpenditure").change(function () {
    ...
    ////console.log(hdnFyExpenditureId);
...
function cellAmntChngeForExpenditureBudget(input) {
   ...
   // var activityAmount = 0;
    ...
    $('#tblSMME tbody tr').each(function () {
        projAmnt = parseFloat($(this).find("td:eq(3) input[type='text']").val()) || 0;
        expendiAmnt = parseFloat($(this).find("td:eq(4) input[type='text']").val()) || 0;// a = 1000      c = a-b = 500
...
function retriveFundDistribution(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //totalUnallocatedBudget = (parseCurrency(totalbudget) - totalfund);
            //$('#totalUnallocatedBudget').html(totalUnallocatedBudget, '#totalUnallocatedBudget');
...
function fundWiseBudgetActivityDetailsForSMME(Id, acId, pbdId) {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectForActivityDetailsForSMMEForExpenditure',                    //// 'SelectForActivityDetailsForSMME',
            param1: 'ProjectId',
            param1Value: parseInt(Id),
            param2: 'ActivityId',
            param2Value: parseInt(acId),
...
function fundWiseBudgetTaskDetails(Id, ActId, taskId) {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectForTaskDetailsForExpenditure',                 /// 'SelectForTaskDetails',
            param1: 'ProjectId',
            param1Value: parseInt(Id),
            param2: 'ActivityId',
...
//function retriveExpenditureReport() { /// Format Report
