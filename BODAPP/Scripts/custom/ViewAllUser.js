...
$(document).ready(function () {
    ...
    DropdownBinder.DDLElem = $("#ddlRoleEnr");
...
$(function () {
    ...
    e.length &&
        e.each(function () {
            var e = $(this);
            e.wrap('<div class="position-relative"></div>'), e.select2({ placeholder: "Select Enterprise", dropdownParent: e.parent() });
        });
                    ...
                    
                    {
                        ...
                        render: function (e, t, a, s) {
                            ...
                            return (
                               ...
                               '<div class="d-flex align-items-center"><a class="btn btn-sm btn-icon" onclick="retriveSingleEdit(' + uid + ',' + k + ')"><i class="ti ti-edit"></i></a></div>'  /// <button class="dropdown-item" onclick="DeleteRecords(\'' + w + '\', \'' + y + '\', ' + a + ')">Delete</button>
                          );
                        },
                    },
                        ],
...
function SaveRecords() {
    ...
    var _data = JSON.stringify({
        User: {
            ...
            UM_Role: 'E',
            UM_SubRoleId: $('#ddlRoleEnr').val(),
            UM_SubRole: $('#ddlRoleEnr option:selected').text(),
            UM_Age: $('#txtAge').val(),
...
function retriveSingleEdit(id,EnrId) {
    //$('#setUPFormUserModal').addClass('show').css('display', 'block').attr('aria-hidden', 'false');
    $('#setUPFormUserModal').modal('show');

    // Close button functionality
    $('.btn-close, .btn[data-bs-dismiss="modal"]').click(function () {
        $('#setUPFormUserModal').removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
        $('.form-control').val('');
        //$('#dvAvatar').text('');
        $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
        $('.select2').val('').change();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#ddlRoleEnr').val(data["UM_SubRole"]).change();
            $('#ddlRoleEnr').val(data["UM_SubRoleId"]).change();
