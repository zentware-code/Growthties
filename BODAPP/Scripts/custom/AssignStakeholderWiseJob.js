                ...
                n.length &&
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            var JobId = row.JD_Id;
                            return (
                                '<a href="javascript:;" class="text-body dropdown-toggle hide-arrow" data-bs-toggle="dropdown" aria-expanded="false"><i class="ti ti-dots-vertical ti-sm mx-1"></i></a><div class="dropdown-menu dropdown-menu-end m-0" style=""><a class="dropdown-item text-black" data-bs-target="#asignShareholder" data-bs-toggle="modal" onclick="showEnterpriseList(' + JobId + ');">Assign</a></div>'
...
function showEnterpriseList(JobId) {
    JD_Id=JobId;
    //alert(SmmeId);
    $("#ulEnterpriseList").html("");
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectEnterpriseForAssignStakeholderWiseJob',
            param1: 'JD_Id',
            param1Value: parseInt(JobId),
            param2: 'EnterpriseId',
            param2Value: parseInt($("#EntrId").val()),
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (i, v) {
                ...
                $("#ulEnterpriseList").append("<li class='d-flex flex-wrap mb-3'>" + logo + "<div class='form-check form-switch'><input class='float-end form-check-input enr'type=checkbox " + v.checked + " id='chkId_" + index + "' onclick='AssignStakeholder("+ index +"," + JobId + "," + v.ENR_Id + ", \"" + v.ENR_CompanyName + "\", \"" + v.ENR_PrimaryContactEmail + "\")'></div>");
               // retriveJob(JobId);
            });
...
function AssignStakeholder( index, JobId, ENR_Id,EnrCompany,EnrEmail) {
    LoaderStart("#asignShareholder");
    var TransactionType = "";
    if ($('#chkId_' + index).is(':checked')) {
        TransactionType = 'InsertStakeHolderJob';
    } else {
        TransactionType = 'UpdateStakeHolderJob';
    }
    ...
    var _data = JSON.stringify({
        entity: {
            TransactionType: TransactionType,
            SWJ_JobId: JobId,
            SWJ_StakeholderId:ENR_Id
            //PD_Id: ProjId,
        }
    }); $.ajax({
        type: "POST",
        url: '/ScriptJson/AssignJobToStakeholder',
        data: _data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
...
function SendMail(EnrCompany, EnrEmail) {
    //$('#btnSave').prop('disabled', true);
    //$("#btnSave").html('Please Wait.....');

    var action = 'assignjobtostakeholder';
