var formElem = document.getElementById("formEnterpriseCustomer");
...
function BindGrid() {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectListForEnterpriseForStakeholder',
            param1: 'CD_EnterpriseId',
            param1Value:parseInt($("#hdnEntId").val()),
    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            v = "/Stakeholder/AddCustomerDetails?Id=";
                            ...
                            return (
                                ...
                                (o
                                    ...
                                    : '<span class="avatar-initial rounded-circle bg-label-' +
                                      ...
                                      "</span>") +
                                '</div></div><div class="d-flex flex-column"><a href="/Stakeholder/CustomerDetails?Id='+a.CD_Id+'" /> <span class="fw-medium">' +
                                 n +
...
function SaveRecords() {
    ...
    var _data = JSON.stringify({
        cust: {
           ...
           // CD_EnterpriseId: $('#ddlBusinessType').val(),
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null && data != undefined && data.IsSuccess == true) {
                ...
                oTable.destroy();
                BindGrid();
                $('#setUPFormEnterpriseCustomerModal').modal('hide');
                $('#setUPFormEnterpriseCustomerModal').find('.form-control').val('');
                $('#setUPFormEnterpriseCustomerModal').find('.select2').val('').change();
...
function retrive(id) {
    $('#setUPFormEnterpriseCustomerModal').modal('show');

    // Close button functionality
    $('.btn-close, .btn[data-bs-dismiss="modal"]').click(function () {
        $('#setUPFormEnterpriseCustomerModal').removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
        $('.form-control').val('');
        $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
        $('.select2').val('').change();
    });
