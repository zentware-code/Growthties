        ...
        success: function (data, status) {
            ...
            i.length && (i = i).wrap('<div class="position-relative"></div>').select2({ placeholder: "Select Country", dropdownParent: i.parent() }),
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "SMME_CompanyName" }, { data: "ASS_CreatedDate" }, { data: "AASS_IsVerified" }, { data: "SMME_PrimaryContactNo" }, { data: "VIEW RESPONSES" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
                      ...
                      
                    {
                        ...
                        render: function (data,type,row) {
                            return '<div class="d-flex align-items-sm-center"><a class="btn btn-sm btn-icon btnEdit ' + row.ViewAns + '" href="/Assessment/ViewSMMEWiseAssessment?BAId=' + row.AAS_BA_Id + '&Id=' + row.SMME_Id + '&AId=' + AId + '"  ><i class="ti ti-eye mx-2 ti-sm"></i></a></div>';
