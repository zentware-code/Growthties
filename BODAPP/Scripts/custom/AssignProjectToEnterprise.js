...
function BindGrid() {
    ...
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectEnterpriseForProjectAssign',
            Param: 'ENR_Id',
        ...
        success: function (data, status) {
            ...
            i.length && (i = i).wrap('<div class="position-relative"></div>').select2({ placeholder: "Select Country", dropdownParent: i.parent() }),
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "ENR_CompanyName" }, { data: "ETM_EnterpriseType" }, { data: "ENR_RegNumber" }, { data: "ENR_IncorporationDate" }, { data: "ENR_PrimaryContactNo" }, { data: "" }, { data: "Assign" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
                    ...
                    },
                    {
                        ...
                        render: function (e, t, a, s) {
                            a = a.ENR_Id;
                            return (
                                '<div class="d-flex align-items-center"><span class="text-nowrap"><button class="btn btn-sm btn-icon me-2"  data-bs-target="#shareProject" data-bs-toggle="modal" onclick="ShowAllProject(' + a + ')" data-bs-dismiss="modal"><i class="ti ti-presentation"></i></button></span></div></div>'
                            );
                        },
                    },
                        ],
...
function ShowAllProject(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.length <= 0) {
                $("#ulProjectList").html("<h5 class='text-muted text-center'>No Unassigned Projects Found.</h5>");
            } else {
...
function AssignProject(index,PdId) {
    ...
    var _data = JSON.stringify({
        entity: {
            PD_EnterpriseId: EntId,
            PD_Id: PdId,
        }
    }); $.ajax({
        type: "POST",
        url: '/ScriptJson/UpdateAssignProject',
        data: _data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
