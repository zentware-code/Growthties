...
function BindGrid() {
    ...
    {
        TranType = 'SelectListForStakeholderUser';
    }
    else{
        TranType = 'SelectListForStakeholder';
    }

    var _data = JSON.stringify({
        global: {
            TransactionType: TranType,
            param1: 'PD_EnterpriseId',
    ...
    $.ajax({
        ...
        dataType: "json",
        beforeSend: function () {
            LoaderStart(".card-datatable");
        },
        complete: function () {
            LoaderEnd(".card-datatable");
        },
        success: function (data, status) {
            data = JSON.parse(data);
            var v = '/Stakeholder/ProjectDetailsNew?Id=';
            var Mval = "";
            if (EnterId > 0) {
                Mval = 'E';
            }
                ...
                columnDefs: [
                         {
                             ...
                             render: function (data, type, row) {
                                 ...
                                 var url = '/Stakeholder/ActivityDescription?Id=' + projectId + '&CAId=' + activityId;
                         ...
                         },
                        {
                            ...
                            render: function (e, t, a, s) {
                                var m = a.PD_Id;
                                return (
                                                            '<div class="d-flex justify-content-start align-items-center user-name"><div class="d-flex flex-column"><a href="#" target="_blank"><span class=" fw-normal text-' +
                        ...
                        },
                         {
                             ...
                             title: "TaskCount",
                             render: function (data, type, row) {
                                 return '<a href="/Stakeholder/TaskList?Id=' + row.CA_ProjectId + '&CAId=' + row.CA_Id + '" class="text-primary" target="_blank">' + row.TaskCount + '</a>';
                             },
                         },
                        {
                            targets: 5,
                            ...
                            render: function (data, type, row) {
                                ...
                                return '<div class="d-flex align-items-sm-center justify-content-sm-center"><a class="btn btn-sm btn-icon btnEdit" href="/Stakeholder/CreateActivity?M=' + Mval + '&Id=' + row.CA_ProjectId + '&CAId=' + row.CA_Id + '"  ><i class="ti ti-edit"></i></a><a href="javascript:;" class="text-body dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="ti ti-dots-vertical ti-sm mx-1"></i></a><div class="dropdown-menu dropdown-menu-end m-0"><a href="/Stakeholder/CreateTask?M=' + Mval + '&Id=' + row.CA_ProjectId + '&CAId=' + row.CA_Id + '" class="dropdown-item" >Create Task</a><a href="/Stakeholder/TaskList?Id=' + row.CA_ProjectId + '&CAId=' + row.CA_Id + '" class="dropdown-item" target="_blank">View Task</a>' + (row.IsCompleted == 1 ? '' : "<a  " + row.VisibilityNone + "  class='dropdown-item text-black' onclick='ActivityCompleted(" + row.CA_Id + ")'> Mark as Completed </a>") + '</div></div>';
                            },
                        },
                ],
                order: [0, "desc"],
