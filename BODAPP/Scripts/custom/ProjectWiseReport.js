...
function fnProjectForAdmin() {
    ...
    $.ajax({
        type: "POST",
        url: '/ScriptJson/GetAdminReportForProjectList',
        ...
        success: function(data) {
           // var projects = JSON.parse(data);
            ...
            //console.log('SMME data:', projects);
...
function populateTable(data) {
    ...
    data.forEach(function (project) {
        var projectLink = '/Project/ProjectDetailsNew?Id=' + project.PD_Id;

        tableRows += '<tr>' +
            '<td><a href="' + projectLink + '">' + project.PD_ProjectName + '</a></td>' +
            '<td>' + project.PD_DurationFromDate + ' - ' + project.PD_DurationToDate + '</td>' +
            '<td>' + project.ENR_CompanyName + '</td>' +
            '<td>' + project.ENR_PrimaryContactEmail + '</td>' +
            '<td>' + project.ProjectDaysLeft + ' Days</td>' +
            '<td>' + formatNumber(project.PD_Budget) + '</td>' +
        '</tr>';
    });
    ...
    $.ajax({
        ...
        success: function (data) {
           ...
           // //console.log('Conlole data', data);
            ...
            var formattedTotalBudget = formatNumber(data["TotalBudget"]);
            var formattedAllocatedForMSMEs = formatNumber(data["TotalSMMEWiseBudget"]);
            var formattedAllocatedForActivities = formatNumber(data["TotalActivityWiseBudget"]);
            var formattedAllocatedForTasks = formatNumber(data["TotalTaskWiseBudget"]);

            ////BUDGET     -Set the formatted values
            $("#spnTotalBudget").text(formattedTotalBudget);
            $("#spnTotalBudgetAllocatedForMSMEs").text(formattedAllocatedForMSMEs);
            $("#spnTotalBudgetAllocatedForActivities").text(formattedAllocatedForActivities);
            $("#spnTotalBudgetAllocatedForTasks").text(formattedAllocatedForTasks);
            ...
            //$("#spnTotalInactiveMSME").text(data["TotalDeactiveSMME"]);
            ...
            //TASKS
            $("#spnTotalTASKS").text(data["TotalTask"]);
            $("#spnPendingTASKS").text(data["TotalOpenTask"]);
            $("#spnInprogressTASKS").text(data["TotalProgressTask"]);
            $("#spnCompletedTASKS").text(data["TotalCompletedTask"]);

            //ACIVITY
            $("#spnTotalACTIVITIES").text(data["TotalActivity"]);
...
(function () {
    ...
    tagify.on('remove', function (e) {
        ...
        fnReportProjectAfterRemoveTag(remainingTags, tagName);
        fnReportProjectAfterRemoveTagForProjectList(remainingTags, tagName);
    });
...
function fnReportProjectAfterRemoveTag(remainingTags,tagName) {
    ...
    $.ajax({
        ...
        //    LoaderEnd("#task-div");
        //    LoaderEnd("#datatable-example");
        //    LoaderEnd("#budget-div-smme");
        //    LoaderEnd("#budget-div");
        //},
        //complete: function () {
        //    LoaderEnd("#main-report");
        //    LoaderEnd("#project-div");
...
function fnReportProjectAfterRemoveTagForProjectList(remainingTags,tagName) {
