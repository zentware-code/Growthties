...
function fnProjectForAdmin() {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectAllStakeholderProjectForAdmin',
            param1: 'PD_StakeholderId',
...
function showEnterpriseList(id, PdId) {
        ...
        success: function (data) {
          ...
          var isenable=0;
            $.each(data, function (i, v) {
                ...
                //$("#ulProjectList").append("<li class='d-flex flex-wrap mb-3'><div class='d-flex flex-grow-1 justify-content-between'><div class=me-2><h6 class=mb-0>" + v.PD_ProjectName + "</h6><p class='mb-0 text-muted'>Duration: " + v.PD_DurationFromDate + "-" + v.PD_DurationToDate + "</div><div class='d-flex align-items-center col-3 justify-content-end'><div class='form-check form-switch'><input class='float-end form-check-input' type='checkbox' id='chkId_" + index + "' onclick='AssignProject(" + index + "," + v.PD_Id + ")'></div></div></div>");
                $("#ulEnterpriseList").append("<li class='d-flex flex-wrap mb-3'>" + logo + "<div class='form-check form-switch'><input class='float-end form-check-input enr'type=checkbox " + v.IsChked + "  " + v.IsEnabled + " id='chkId_" + index + "' onclick='AssignStakeholder(" + PdId + "," + v.ENR_Id + ",this)'></div>");
...
function AssignStakeholder( PdId,ENR_Id,chk) {
    ...
    var _data = JSON.stringify({
        entity: {
            TransactionType: TransactionType,
            PD_Id: PdId,
            PD_StakeholderId:ENR_Id
            //PD_Id: ProjId,
        }
    }); $.ajax({
        type: "POST",
        url: '/ScriptJson/InsertProjectDetails',
        data: _data,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
