...
function fnStakeholderForEnterprise(ProjectId) {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectProjectWiseStakeholderLogo',
            param1: 'PD_Id',
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            //console.log('ENR_LOGO',data);

            StakeholderLogoList = '';
            if (count > 0) {
                $.each(data, function (i, v) {
                    StakeholderLogoList = StakeholderLogoList + v.ENR_Logo;
                });
            }
            else {
                StakeholderLogoList = '<div class="d-flex align-items-center" >No Stakeholder Found</div>';
            }            
        },
        error: function (data) {
            Swal.fire({
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (i, v) {
                ...
                if (count > 0) {
                    fnStakeholderForEnterprise(v.PD_Id);
...
function showEnterpriseList( PdId) {
   ...
   // EntId = id;
    ...
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectEnterpriseForStakeholder',
...
function AssignStakeholder( index,PdId,ENR_Id) {
  ...
  //  var TransactionType = 'InsertStakeHolder';
    //if($(chk).is(':checked'))
