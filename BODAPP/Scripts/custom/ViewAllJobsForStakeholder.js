...
function BindGrid() {
    ...
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectAllJobForStakeholder',
            param1: 'JD_EnterpriseId',
            param1Value: $('#EntrId').val(),
            param2: 'SWJ_StakeholderId',
            param2Value: $('#StakeHolderId').val(),
            param3: 'JD_CustomerId',
            param3Value: parseInt(CustId),
                ...
                n.length &&
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            var List = '';  // Initialize the List variable

                            // Handling for Single Visit Type
                            if (row.JD_VistType == "Single") {
                                var Link = '';

                                // Conditions for the different OpenStatus states
                                ...
                                // Constructing the dropdown menu for Single
                                List = '<a href="#" class="text-body dropdown-toggle hide-arrow" data-bs-toggle="dropdown" aria-expanded="false"><i class="ti ti-dots-vertical ti-sm mx-1"></i></a>' +
                                       '<ul class="dropdown-menu dropdown-menu-end m-0">' + Link + '</ul>';
                            }

                            // Handling for Multiple Visit Type
                            if (row.JD_VistType == "Multiple" && row.JD_Status != "Completed") {
                                retriveMultiplejob(row.JD_Id, row.JD_VistType, row.OpenStatus);
                            }

                            // Handling for Maintenance Visit Type
                            if (row.JD_VistType == "Maintenance" && row.JD_Status != "Completed") {
                                retriveMaintenancejob(row.JD_Id, row.JD_VistType, row.OpenStatus);
                            }

                            // Final Return Structure for Rendering
                            return (
                                '<div class="d-flex align-items-center">' +
                                '<a href="' + v + row.JD_Id + '" class="text-body" target="_blank"><i class="ti ti-edit ti-sm me-2"></i></a>' +
                                '<button class="btn btn-sm btn-icon me-2" data-bs-target="#shareProject" data-bs-toggle="modal" onclick="ShowAllSMME(' + row.JD_Id + ')" data-bs-dismiss="modal">' +
                                '<i class="ti ti-user-plus ti-sm me-2"></i></button>' +
                                List +  // Dropdown menu here
                                '</div>'
                            );
                        }
                    },
