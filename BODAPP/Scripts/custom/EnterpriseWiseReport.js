...
function EnterpriseList() {
    ...
    $.ajax({
        type: "POST",
        url: '/ScriptJson/GetAdminReportForEnterpriseList',
        ...
        success: function (data, status) {
            ...
            var tableBody = '';
            var v = "/Home/EnterpriseProfieView_Profiles?Id=";
            var items = data.EnterpriseList;
            if (Array.isArray(items)) {
                $.each(items, function (index, item) {
                    var companyName = item.ENR_CompanyName || '';
                    var primaryContactEmail = item.ENR_PrimaryContactEmail || '';
                    var logo = item.ENR_Logo || '';
                    var ENR_Id = item.ENR_Id || '';
                    ...
                    avatar += '</div></div><div class="d-flex flex-column"><a href="' + v + ENR_Id + '" class="text-body text-truncate"><span class="fw-medium">' + companyName + '</span></a><small class="text-muted">' + primaryContactEmail + '</small></div></div>';

                    var incorporationDate = '';
                    if (item.ENR_IncorporationDate) {
                        var timestamp = item.ENR_IncorporationDate.replace(/\/Date\((\d+)\)\//, '$1');
                    ...
                    tableBody += '<tr>' +
                        '<td>' + avatar + '</td>' +
                        '<td>' + item.ETM_EnterpriseType + '</td>' +
                        '<td>' + item.ENR_RegNumber + '</td>' +
                        '<td>' + incorporationDate + '</td>' +
                        '<td>' + item.ENR_PrimaryContactNo + '</td>' +
                        '<td><span class="badge ' + (item.ENR_Class || '') + '">' + (item.ENR_Active || '') + '</span></td>' +
                        '</tr>';
                });
            } else {
                console.error("Expected an array in data.EnterpriseList, but got:", items);
            }
...
(function () {
    ...
    tagify.on('remove', function (e) {
        ...
        fnReportAdminAfterRemoveTagENRList(remainingTags, tagName);
    });
...
function fnReportAdminAfterRemoveTag(remainingTags,tagName) {
    ...
    $.ajax({
        ...
        //beforeSend: function () {
        //    LoaderStart("#enterprise-div");
        //    LoaderStart("#datatable-example");
        //},
        //complete: function () {
        //    LoaderEnd("#enterprise-div");
...
function fnReportAdminAfterRemoveTagENRList(remainingTags, tagName) {
...
function fnReportAdmin() {
    ...
    // var _data = JSON.stringify({
    //    rpt: {
    //        CountryId: parseInt($('#hdnddlCountry').val()),
    //        ProvinceId: parseInt($('#hdnddlProvince').val()),
    //        IndustrySectorId: parseInt($('#hdnddlIndustrySector').val()),
    //        BusinessTypeId: parseInt($('#hdnddlBusinessType').val()),
    //        LegalEntityTypeId: parseInt($('#hdnddlLegalEntity').val()),
    //        ProtfolioGrowthId: parseInt($('#hdnddlProtfolioGrowth').val()),
    //        JobCategoryId: parseInt($('#hdnddlJobCategory').val()),
    //        CustomerTypeId: parseInt($('#hdnddlCustomerType').val()),
    //        BudgetTypeId: parseInt($('#hdnddlBudgetType').val()),
    //        FromDate: FromDate,
    //        ToDate: ToDate,
    //    }
    //});
