...
!(async function () {
    ...
    var kan=  JSON.parse(localStorage.getItem('kanban'));

    var c = [];
    for (var ix = 1; ix < kan.length; ix++) {
        c.push({
            id: kan[ix].id,
            title: kan[ix].title,
            item: JSON.parse(kan[ix].item),
        ...
        return null == e
            ...
            : e
                  ...
                  .map(function (e, t, a) {
                      ...
                      return "<div class='avatar " + r + a + "'data-bs-toggle='tooltip' data-bs-placement='top'title='" + o[t] + "'><img src='" + l + "img/avatars/" + e + "' alt='Avatar' class='rounded-circle " + d + "'></div>";
                  })
                  .join(" ");
    }
    //c.ok || console.error("error", c),
    //(c = await c.json()),
    d && d.flatpickr({ monthSelectorType: "static", altInput: !0, altFormat: "j F, Y", dateFormat: "Y-m-d" }),
    o.length &&
        o.each(function () {
            var e = $(this);
    ...
    t && new Quill(t, { modules: { toolbar: ".comment-toolbar" }, placeholder: "Write a Comment... ", theme: "snow" });
    const v = new jKanban({
        ...
        itemAddOptions: { enabled: !0, content: "+ Add New Item", class: "kanban-title-button btn", footer: !1 },
        click: function (e) {
            var t = e,
                ...
                i = new Date(),
                d = i.getFullYear(),
                n = n ? n + ", " + d : i.getDate() + " " + i.toLocaleString("en", { month: "long" }) + ", " + d,
            ...
            s.show(),
                ...
                r
                    ...
                    .insertAdjacentHTML(
                        "afterbegin",
                        p(d, !1, "xs", "1", e.getAttribute("data-members")) + "<div class='avatar avatar-xs ms-1'><span class='avatar-initial rounded-circle bg-label-secondary'><i class='ti ti-plus ti-xs text-heading'></i></span></div>"
                    );
        },
        buttonClick: function (e, a) {
            ...
            n.setAttribute("class", "new-item-form"),
                (n.innerHTML =
                    '<div class="mb-3"><textarea class="form-control add-new-item" rows="2" placeholder="Add Content" autofocus required></textarea></div><div class="mb-3"><button type="submit" class="btn btn-primary btn-sm me-2 waves-effect waves-light">Add</button><button type="button" class="btn btn-label-secondary btn-sm cancel-add-item waves-effect waves-light">Cancel</button></div>'),
                v.addForm(a, n),
                n.addEventListener("submit", function (e) {
                    e.preventDefault();
                    var t = [].slice.call(document.querySelectorAll(".kanban-board[data-id=" + a + "] .kanban-item"));
                    v.addElement(a, { title: "<span class='kanban-text'>" + e.target[0].value + "</span>", id: a + "-" + t.length + 1 });
        ...
        g = (e && new PerfectScrollbar(e), document.querySelector(".kanban-container")),
    ...
    k &&
        k.forEach(function (e) {
            ...
            let i = "";
            null !== e.getAttribute("data-image") && (i = "<img class='img-fluid rounded mb-2' src='" + l + "img/elements/" + e.getAttribute("data-image") + "'>"),
                (e.textContent = ""),
                void 0 !== e.getAttribute("data-badge") &&
                    ...
                    e.insertAdjacentHTML(
                        ...
                        "<div class='d-flex justify-content-between flex-wrap align-items-center mb-2 pb-1'><div class='item-badges'> <div class='badge rounded-pill bg-label-" + t + "'> " + a + "</div></div>" + m() + "</div>" + i + n)
                    ),
                (void 0 === e.getAttribute("data-comments") && void 0 === e.getAttribute("data-due-date") && void 0 === e.getAttribute("data-assigned")) ||
                    e.insertAdjacentHTML(
                        "beforeend",
...
a &&
    a.addEventListener("submit", function (e) {
        ...
        var e = this.querySelector(".form-control").value,
            ...
            t = (v.addBoards([{ id: t, title: e }]), document.querySelectorAll(".kanban-board:last-child")[0]),
            e =
                (t &&
                    (t.querySelector(".kanban-title-board").insertAdjacentHTML("afterend", b()),
...
r &&
    r.addEventListener("shown.bs.offcanvas", function () {
        [].slice.call(r.querySelectorAll('[data-bs-toggle="tooltip"]')).map(function (e) {
            return new bootstrap.Tooltip(e);
        });
    });
})();
