var SMMELogoList = '';
let selectedStakeholderValue = null;
...
function BindGrid() {
    ...
    //var _data = JSON.stringify({
    //    global: {
    //        TransactionType: 'SelectAllProject',  SelectProjectForEnterprise
    //       // Param: 'PD_Id',
    //        StoreProcedure: 'ProjectDetails_USP',
    //    }
    //});
    $.ajax({
        ...
        success: function (data, status) {
            ...
            v = "/Project/ProjectDetailsNew?Id=";
...
function showEnterpriseList(PdId) {
    ...
    var _data = JSON.stringify({
        global: {
            ...
            param2: 'EnterpriseId',
            param2Value: parseInt($("#EnterpriseId").val()),
            StoreProcedure: 'ProjectDetails_USP'
        }
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (i, v) {
                ...
                $("#ulEnterpriseList").append("<li class='d-flex flex-wrap mb-3'>" + logo + "<div class='form-check form-switch'><input class='float-end form-check-input enr'type=checkbox " + v.checked + " id='chkId_" + index + "' onclick='AssignStakeholder("+ index +"," + PdId + "," + v.ENR_Id + ", \"" + v.ENR_CompanyName + "\", \"" + v.ENR_PrimaryContactEmail + "\")'></div>");
...
function retriveStakeholderRole(enrId, EnrCompany, EnrEmail) {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'selectStakeholderRole',
            param1: 'ENR_Id',
    ...
    $.ajax({
        ...
        success: function (data) {
            data = JSON.parse(data);
            $('#StakeholderUserRole').val(data[0].UM_SubRole);
            //console.log('role',data.UM_SubRole)
            SendMail(EnrCompany, EnrEmail,data[0].UM_SubRole);
        },
        error: function (data) {
            Swal.fire({
                title: "Oops...",
...
function AssignStakeholder( index, PdId, ENR_Id,EnrCompany,EnrEmail) {
    var TransactionType = "";
    if ($('#chkId_' + index).is(':checked')) {
        retriveStakeholderRole(ENR_Id,EnrCompany,EnrEmail);
    ...
    var _data = JSON.stringify({
        entity: {
            ...
            PWS_StakeholderTypeId: selectedStakeholderValue,
        }
    }); $.ajax({
        type: "POST",
...
function SendMail(EnrCompany, EnrEmail, role) {
    //$('#btnSave').prop('disabled', true);
    //$("#btnSave").html('Please Wait.....');
    LoaderStart("#asignShareholder");
    var action = 'assignprojecttostakeholder';
...
// Select only the first 5 checkboxes
const topFiveCheckboxes = $('.form-check-input').slice(0, 5);

topFiveCheckboxes.on('change', function () {
    if ($(this).is(':checked')) {
        selectedStakeholderValue = $(this).val();
        topFiveCheckboxes.not(this).prop('disabled', true);

        console.log("Selected Stakeholder Value:", selectedStakeholderValue);
    } else {
        // Re-enable all top five if unchecked
        topFiveCheckboxes.prop('disabled', false);
        selectedStakeholderValue = null;

        console.log("No Stakeholder Selected");
    }
});
