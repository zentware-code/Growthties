                    ...
                    columnDefs: [

                        {
                            ...
                            render: function (t, e, s, a) {
                                return '<input type="checkbox" id="chkGrid_'+ s.QN_Id +'" class="dt-checkboxes form-check-input selectval" ' + s.checked + ' onClick="get(this,' + s.QN_Id + ',\''+s.QN_QuestionType+'\')" value="' + s.checkedboxval + '" >';
                            },
                            //render: function (t, e, s, a) {
                            //    return '<input type="checkbox" id="chkGrid_' + s.QN_Id + '" class="dt-checkboxes form-check-input selectval" ' +
                            //           s.checked + ' onClick="get(this,' + s.QN_Id + ',\'' + s.QN_QuestionType + '\')" value="' + s.QN_Id + '" >';
                            //},
...
function setchkall(o) {
    ...
    if (isChecked) {
        ...
        rowcollection.each(function (index, elem) {
            ...
            var questionId = elem.id;
           
            var questionId = elem.id.split("_");
            if (questionId[1] && questionId[1] !== "0") {
                var _data = JSON.stringify({
                    global: {
                        TransactionType: 'SelectedQuestionSingle',
                        param1: 'QN_Id',
                        param1Value: parseInt(questionId[1]),
                        StoreProcedure: 'BuildAssessmentSetUp_USP'
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        QuesArr.push({
                            BAD_QuestionId: questionId[1],
                                BAD_Question: data[0].QN_Question,
                                BAD_QuestionOption: data[0].QN_QuestionType
                        });
...
{      
    ...
    if ((checkArray(1, yourArray))=='Y' && $('#ddlCategory').val()>0) {
        ...
        Swal.fire({
            title: "Please Check alteast one Question!",
            icon: "error",
            customClass: { confirmButton: "btn btn-primary waves-effect waves-light" },
            buttonsStyling: !1
        });
