...
var formElem = document.getElementById("formSMMEUserManage");
        ...
        success: function (data, status) {
            ...
            //i.length && (i = i).wrap('<div class="position-relative"></div>').select2({ placeholder: "Select Country", dropdownParent: i.parent() }),
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "UM_Name" }, { data: "UM_ContactNo" }, { data: "UM_SubRole" }, { data: "SMME_CompanyName" }, { data: "SMME_PrimaryContactEmail" }, { data: "action" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
                            ...
                            return (
                                ...
                                (o
                                    ...
                                    : '<span class="avatar-initial rounded-circle bg-label-' +
                                      ...
                                      "</span>") +
                                '</div></div><div class="d-flex flex-column"><a onclick="getValue(105)" href="' + link + '"><span class="fw-medium">' +
                                n +
        ...
        success: function (data) {
            if (data != null && data != undefined && data.IsSuccess == true) {
                ...
                if (retId > 0) {
                    ...
                    oTable.destroy();
                    BindGrid();
                    $('#setUPFormSMMEUserModal').modal('hide');
                    $('#setUPFormSMMEUserModal').find('input, select').val('');
                    $('#ddlRole').find('option').remove();
                    $('#ddlGender').find('option').remove();
                    $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
                    $(".form-control").val('')
...
function show() {
    ...
    $('#setUPFormSMMEUserModal').addClass('show');
    $('.btn-close').click(function () {

        $('#setUPFormSMMEUserModal').removeClass('show');
        $('.form-control').val('');
    });
...
function retrive(id) {
    $('#setUPFormSMMEUserModal').modal('show');

    // Close button functionality
    $('.btn-close, .btn[data-bs-dismiss="modal"]').click(function () {
        $('#setUPFormSMMEUserModal').removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
        $('.form-control').val('');
        $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
        $('.select2').val('').change();
    });
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (data) {
            console.log('SMME User data-',data);
            //data = JSON.parse(data);
            $('#Id').val(data["UM_Id"]);
