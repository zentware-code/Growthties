var formElem = document.getElementById("formSMMECustomer");
...
$("#ddlSector").change(function () {
    ...
    l.length &&
      l.each(function () {
          var l = $(this);
          l.wrap('<div class="position-relative"></div>'), l.select2({ placeholder: "Select A Business Type", dropdownParent: l.parent() });
      });
...
function BindGrid() {
    var _data = JSON.stringify({
        global: {
           TransactionType: 'SelectListForSMME',
           param1: 'CD_SmmeId',
           param1Value:parseInt($("#hdnCustSmmeId").val()),
        ...
        success: function (data, status) {
            ...
            v = "/Customer/AddCustomerDetails?Id=";
                ...
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "CD_Name" }, { data: "CD_ContactNumber" }, { data: "CD_Type" }, { data: "CD_RegistrationNo" }, { data: "action" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
...
function SaveRecords() {
    ...
    var _data = JSON.stringify({
        cust: {
            ...
            CD_BusinessTypeId: $('#ddlBusinessType').val(),
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null && data != undefined && data.IsSuccess == true) {
                ...
                oTable.destroy();
                BindGrid();
                $('#setUPFormSMMECustomerModal').modal('hide');
               // $('#setUPFormSMMECustomerModal').find('input,  select').val('');
                $('#setUPFormSMMECustomerModal').find('.form-control').val('');
                $('#setUPFormSMMECustomerModal').find('.select2').val('').change();
...
function retrive(id) {
    $('#setUPFormSMMECustomerModal').modal('show');

    // Close button functionality
    $('.btn-close, .btn[data-bs-dismiss="modal"]').click(function () {
        $('#setUPFormSMMECustomerModal').removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
        $('.form-control').val('');
        $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
        $('.select2').val('').change();
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            DropdownBinder.DDLData = {
                ...
                Text: 'ETM_EnterpriseType',
                Value: 'ETM_Id',
                PId: data["CD_SectorId"],
                ColumnName: 'ETM_IndustryId'
            };
            DropdownBinder.DDLElem = $("#ddlBusinessType");
            DropdownBinder.Execute();

            $('#ddlBusinessType').val(data["CD_BusinessTypeId"]).change();
...
//            'Authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..ytb5PO52KK8ij-UZzqBpPp0zUN3fIpW56l4HsMa3d2w'
//        },
