...
let radialGrph = (divSelector, completedAssessment) => {
    //var completedAssessment = parseFloat($('#completedAssessment').val());
    var radialData = completedAssessment.toFixed(2);
...
function retriveProjectList() {
    ...
    if ($("#hdnUserType").val() == "EnrUser") {
        var postData = JSON.stringify({
            global: {
                TransactionType: 'SelectProjectsListForEnrDashboardForUserAssigned',
                param1: 'UWP_UserId',
        ...
        var postData = JSON.stringify({
            global: {
                TransactionType: "SelectProjectsListForEnrDashboard",
                param1: "EnterpriseId",
                param1Value: EnterId,
                StoreProcedure: "ProjectDetails_USP"
            }
                            ...
                            render: function (data, type, row) {
                                ...
                                return '<div class="d-flex justify-content-left align-items-center">' +
                                           ...
                                           '<div class="d-flex flex-column">' +
                                               '<span class="text-truncate fw-medium"><a style="width: 125px;white-space: nowrap;overflow: hidden !important;text-overflow: ellipsis;" href="/Project/ProjectDetailsNew?Id=' + Pd_Id + '" onclick="getValue(68)">' + name + '</a></span>' +
                            ...
                            render: function (data, type, row) {
                                ...
                                return '<div class="d-inline-block">' +
                                           ...
                                           '<div class="dropdown-menu dropdown-menu-end m-0">' +
                                               '<a href="/Project/ProjectDetails?Id=' + Pd_Id + '" class="dropdown-item">Edit</a>' +
                                               '<a href="/Project/ProjectDetailsNew?Id=' + Pd_Id + '" class="dropdown-item" onclick="getValue(68)">View</a>' +
                                           '</div>' +
                                       '</div>';
                            }
                        }
...
function retriveEnrDashboardData() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#totalAssessments').text(data.totalAssessments);
            $('#totalProjects').text(data.totalProjects);
            $('#totalActivities').text(data.totalActivities);
            $('#totalTasks').text(data.totalTasks);
            $('#totalSMMEs').text(data.totalSMMEs);

            $('#totalEntities').text((data.totalUsers)+(data.totalAssessments)+(data.totalProjects)+(data.totalActivities)+(data.totalTasks)+(data.totalSMMEs));
