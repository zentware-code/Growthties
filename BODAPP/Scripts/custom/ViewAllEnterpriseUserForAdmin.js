...
var formElem = document.getElementById('formEnterpriseUserForAdmin');
...
$(function () {
    ...
    t.length &&
       t.each(function () {
           var t = $(this);
           t.wrap('<div class="position-relative"></div>'), t.select2({ placeholder: "Select ID Type", dropdownParent: t.parent() });
       });
    ...
    FormValidation.formValidation(formElem, {
        fields: {
            ...
            txtAge: {
                validators: {
                    notEmpty: {
                        message: "Please fill user date of birth"
                    }
                }
            },
            ddlGender: {
...
function retrive(id,EnrId) {
    $('#setUPFormEnterpriseUserForAdminModal').modal('show');
    // Close button functionality
    $('.btn-close, .btn[data-bs-dismiss="modal"]').click(function () {
        $('#setUPFormEnterpriseUserForAdminModal').removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
        $('.form-control').val('');
        $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
        $('.select2').val('').change();
    });
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#ddlBranch').val(data["UM_BranchId"]).change();

            $('#ddlPrimaryIdType').val(data["UM_PrimaryIdType"]).change();
            $('#txtPrimaryIdNumber').val(data["UM_PrimaryIdNumber"]);
...
function SaveRecords() {
    ...
    var _data = JSON.stringify({
        User: {
            UM_Id: $('#Id').val(),
            UM_FirstName: $('#txtFirstName').val(),
            UM_LastName: $('#txtLastName').val(),
            ...
            UM_PrimaryIdType: $('#ddlPrimaryIdType').val(),
            UM_PrimaryIdNumber: $('#txtPrimaryIdNumber').val()
        }
    });
    $.ajax({
        type: "POST",
        ...
        success: function (data) {
            ...
            //    $('#setUPFormEnterpriseUserForAdminModal').find('input, select').val('');
            //    $(".form-control").val('')
...
function BindGrid() {
    var _data = JSON.stringify({
        global: {
            TransactionType: 'SelectAllEnterpriseUserForAdmin',
            param1: 'UM_Role',
    ...
    $.ajax({
        ...
        success: function (data, status) {
            ...
            //p  = "/Enterprise/UserPermission?mainid=1&UserId=";
            v = "/Home/EnterpriseUserForAdmin?Id=";
            ...
            //i.length && (i = i).wrap('<div class="position-relative"></div>').select2({ placeholder: "Select Country", dropdownParent: i.parent() }),
                n.length &&
                    (e = n.DataTable({
                        data:data,
                        columns: [{ data: "" }, { data: "UM_Name" }, { data: "UM_ContactNo" }, { data: "UM_SubRole" }, { data: "ENR_CompanyName" }, { data: "UM_EmailId" }, { data: "action" }],
                        columnDefs: [
                    {
                        className: "control",
                        searchable: !1,
                    ...
                    
                    {
                        ...
                        render: function (e, t, a, s) {
                            ...
                            var k = a.UM_MainId;
                            return (
                               '<div class="d-flex align-items-center"><a href="#" target="blank" class="text-body dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="ti ti-dots-vertical ti-sm mx-1"></i></a><div class="dropdown-menu dropdown-menu-end m-0"><button class="dropdown-item" onclick="retrive(' + id + ',' + k + ')">Edit</i></button><a href="/Enterprise/UserPermission?mainid=' + k + '&UserId=' + id + '" class="dropdown-item">Manage Permissions</a></div></div>'
                            );
                        },
                    },
                        ],
                        ...
                        initComplete: function () {
                            this.api()
                                ...
                                .every(function () {
                                    var t = this,
                                        a = $('<select id="UserRole" class="form-select text-capitalize"><option value=""> Select User Role </option></select>')
                                            .appendTo(".user_role")
                                            .on("change", function () {
...
function SendMail() {
    ...
    var action = 'enterpriseuseradd';

    var _data = JSON.stringify({
        emailcontent: {
            UserName: $('#txtFirstName').val(),
            SubRole: $('#ddlRoleEnr option:selected').text(),
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != null && data.IsSuccess === true) {
                ...
                $('#ddlRoleEnr').find('input, select').val('');
                $('#ddlGender').find('input, select').val('');
                $('#prfpicIMG').attr('src', '/Content/assets/img/avatars/userpic.png');
